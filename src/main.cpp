...
float meanFilteredValue, meanScaledValue, sensitiveFilteredValue, sensitiveScaledValue;
SensorState state;
...
void controlInverter()
{
 ...
 CANBrakeState[0]=CANBrakeState[1];
...
void appsTesting(){
  ...
  apps_p->update(raw1, raw2, meanFilteredValue, meanScaledValue, sensitiveFilteredValue, sensitiveScaledValue, state);
  ...
  if (apps_p->getSensorState() == SensorState::NORMAL) {
    ...
    Serial.print(apps_p->getMeanScaledValue(), 1);
    Serial.println("%");
  } else {
    Serial.print("FAULT! -> ");
