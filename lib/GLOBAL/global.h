...
int32_t stsInverterCAN_EPRM;      // Electrical RPM
int16_t stsInverterCAN_DutyCycle; // Duty cycle in percentage (multiplied by 10)
int16_t stsInverterCAN_InputVoltage; // Input voltage in volts

// Packet ID 0x21: AC Current, DC Current
int16_t stsInverterCAN_ACCurrent; // AC current in Amperes (multiplied by 10)
int16_t stsInverterCAN_DCCurrent; // DC current in Amperes (multiplied by 10)

// Packet ID 0x22: Controller Temp., Motor Temp., Fault code
int16_t stsInverterCAN_ControllerTemp; // Controller temperature in °C (multiplied by 10)
int16_t stsInverterCAN_MotorTemp;      // Motor temperature in °C (multiplied by 10)
uint8_t stsInverterCAN_FaultCode;      // Fault code

// Packet ID 0x23: Id, Iq values
int32_t stsInverterCAN_Id; // FOC algorithm component Id (multiplied by 100)
int32_t stsInverterCAN_Iq; // FOC algorithm component Iq (multiplied by 100)

// Packet ID 0x24: Throttle signal, Brake signal, Digital I/Os, Drive enable, Limit status bits, CAN map version
int8_t stsInverterCAN_ThrottleSignal;   // Throttle signal in percentage
int8_t stsInverterCAN_BrakeSignal;      // Brake signal in percentage
uint8_t stsInverterCAN_DigitalInputs;   // 4 bits representing the digital inputs
uint8_t stsInverterCAN_DigitalOutputs;  // 4 bits representing the digital outputs
uint8_t stsInverterCAN_DriveEnable;     // Drive enable status (0: disabled, 1: enabled)
uint8_t stsInverterCAN_CANMapVersion;   // CAN map version
// Packet ID 0x20: ERPM, Duty, Input Voltage
int32_t stsInverterCAN_20[1];  // Electrical RPM
int16_t stsInverterCAN_20_1[1];  // Duty cycle in percentage (multiplied by 10)
int16_t stsInverterCAN_20_2[1];  // Input voltage in volts

// Packet ID 0x21: AC Current, DC Current
int16_t stsInverterCAN_21[2];  // Array for AC and DC current
// stsInverterCAN_21[0] - AC current in Amperes (multiplied by 10)
// stsInverterCAN_21[1] - DC current in Amperes (multiplied by 10)

// Packet ID 0x22: Controller Temp., Motor Temp., Fault code
int16_t stsInverterCAN_22[2];  // Array for Controller and Motor temperature
...
uint8_t stsInverterCAN_22_FaultCode[1];  // Fault code
byte stsInverterCAN_22_FULL[8];

// Packet ID 0x23: Id, Iq values
int32_t stsInverterCAN_23[2];  // Array for Id and Iq values
...
// Packet ID 0x24: Throttle signal, Brake signal, Digital I/Os, Drive enable, Limit status bits, CAN map version
int8_t stsInverterCAN_24_1[2];  // Array for Throttle and Brake signal
// stsInverterCAN_24_1[0] - Throttle signal in percentage
// stsInverterCAN_24_1[1] - Brake signal in percentage
uint8_t stsInverterCAN_24_DigitalInputs[1];  // Digital Inputs (4 bits)
uint8_t stsInverterCAN_24_DigitalOutputs[1];  // Digital Outputs (4 bits)
uint8_t stsInverterCAN_24_2[1];  // Drive enable status (0: disabled, 1: enabled)
uint8_t stsInverterCAN_24_3[1];  // CAN map version
byte stsInverterCAN_24_FULL[8];

void assignstsInverterCANValues() {
    // Packet ID 0x20: ERPM, Duty, Input Voltage
